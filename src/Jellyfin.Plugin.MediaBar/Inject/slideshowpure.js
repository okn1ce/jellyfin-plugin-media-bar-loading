...
playButton.onclick = async () => {
  ...
  try {
    const sessions = await apiClient.getJSON(apiClient.serverAddress() + "/Sessions");
    if (!sessions.length) {
      console.error("No active Jellyfin sessions found.");
      return;
    }

    // Find the session matching the current user
    const userSession = sessions.find(session => session.UserId === userId);
    if (!userSession) {
      console.error("No active session found for the current user.");
      return;
    }
    ...
    const response = await fetch(playbackUrl, {
      ...
      headers: {
        ...
        Authorization: `DeviceId="unique-device-id", Token="${token}"`,
      },
    });
...
detailButton.innerHTML = `
  ...
  <span class="detail-text">More Info</span>
`;
...
slide.append(
  ...
  infoContainer,
  genreElement,
  buttonContainer // Add button container instead of individual buttons
);
